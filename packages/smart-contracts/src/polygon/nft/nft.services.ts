...
const transferToDev = async (req: any, res: any) => {
    try {
        ...
        const provider = new ethers.JsonRpcProvider(process.env.RPC_ENDPOINT_POLYGON);
        
        const signer = new ethers.Wallet(wallet, provider);
    
        const contract = new ethers.Contract(collectionAddress, ABI, signer);
    
        const txn = await contract.safeTransferFrom(signer.address, process.env.DEV_ADDRESS, tokenId);
